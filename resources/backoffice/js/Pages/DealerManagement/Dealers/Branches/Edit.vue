...
const hydrateInitialSuburb = () => {
    form.suburb_id = normalizeFromOptions(form.suburb_id, suburbsAll.value)
    if (!toKey(form.suburb_id)) return
    syncParentsFromSuburb()
}

hydrateInitialSuburb()

const submit = () => {
    form.patch(route('backoffice.dealer-management.dealers.branches.update', [props.dealer.id, props.data.id]), {
        preserveScroll: true,
    })
}
