...
const fetchRecords = (p, helpers) => {
    router.get(
        route('backoffice.dealer-management.dealers.users', props.dealer.id),
        {
            page: p?.page,
...
const confirmDelete = (row) => {
    confirmAction({
        ...
        cancelLabel: 'Cancel',
        method: 'delete',
        actionUrl: route('backoffice.dealer-management.dealers.users.destroy', [props.dealer.id, row.id]),
...
const confirmResetPassword = (row) => {
    confirmAction({
        ...
        cancelLabel: 'Cancel',
        method: 'post',
        actionUrl: route('backoffice.dealer-management.dealers.users.reset-password', [props.dealer.id, row.id]),
        inertia: { preserveState: true },
    })
}
</script>
...
<template>
    ...
    <NotesHost
        ref="notesRef"
        noteable-type="dealer-user"
        :title-fn="row => row.name || 'Dealer User'"
