...
const currentUrl = computed(() => page.url || route('backoffice.dealer-management.dealers.leads', props.dealer.id))
const canManage = computed(() => !!abilities.value.manageDealershipLeads)
...
const fetchRecords = (p, helpers) => {
  router.get(
    route('backoffice.dealer-management.dealers.leads', props.dealer.id),
    {
      page: p?.page,
...
const confirmDelete = (row) => {
    ...
    cancelLabel: 'Cancel',
    method: 'delete',
      actionUrl: route('backoffice.dealer-management.dealers.leads.destroy', { dealer: props.dealer.id, lead: row.id, return_to: currentUrl.value }),
...
<template>
  ...
  <div class="row nowrap justify-between items-center q-mb-md">
    ...
    <q-btn
      ...
      no-wrap
      unelevated
      @click="router.visit(route('backoffice.dealer-management.dealers.leads.create', { dealer: dealer.id, return_to: currentUrl }))"
    />
  </div>
  ...
  >
    ...
    <template #actions="{ row }">
      ...
      <q-btn v-if="row.can?.view" round dense flat icon="visibility" @click="router.visit(route('backoffice.dealer-management.dealers.leads.overview', { dealer: dealer.id, lead: row.id, return_to: currentUrl }))">
        <q-tooltip>Overview</q-tooltip>
      </q-btn>

      <q-btn v-if="row.can?.edit" round dense flat icon="edit" @click="router.visit(route('backoffice.dealer-management.dealers.leads.edit', { dealer: dealer.id, lead: row.id, return_to: currentUrl }))">
        <q-tooltip>Edit</q-tooltip>
      </q-btn>

      <q-btn v-if="row.can?.delete" round dense flat icon="delete" color="negative" :disable="loading" @click.stop="confirmDelete(row)">
