...
const props = defineProps({ publicTitle: { type: String, default: 'Dealer Management' }, dealer: { type: Object, required: true }, returnTo: { type: String, default: '' }, data: { type: Object, required: true } })

const form = useForm({ return_to: props.returnTo || '', name: props.data?.name || '', is_default: !!props.data?.is_default })

const submit = () => form.patch(route('backoffice.dealer-management.dealers.lead-pipelines.update', { dealer: props.dealer.id, leadPipeline: props.data.id }), { preserveScroll: true })
...
<template>
  ...
  <q-card flat bordered><q-card-section>
    <div class="text-h6 q-pb-lg">Edit Lead Pipeline</div>
    <q-form @submit.prevent="submit">
      <q-input v-model="form.name" filled dense label="Name" :error="!!form.errors.name" :error-message="form.errors.name" />
