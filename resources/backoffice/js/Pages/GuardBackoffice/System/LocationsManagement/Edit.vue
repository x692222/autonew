...
const normalizeOptionValue = (items, value) => {
    const key = toKey(value)
    if (!key) return null

    const option = (items ?? []).find((item) => toKey(item.value) === key)
    return option ? option.value : value
}

const initialCountryId = normalizeOptionValue(props.options?.countries, props.data.country_id ?? null)
const initialStateId = normalizeOptionValue(props.options?.states, props.data.state_id ?? null)
const initialCityId = normalizeOptionValue(props.options?.cities, props.data.city_id ?? null)
...
watch(() => form.country_id, () => {
    if (['city', 'suburb'].includes(props.type)) {
        form.state_id = null
    }
    if (props.type === 'suburb') {
        form.city_id = null
    }
})
...
const submit = () => {
    form.patch(route('backoffice.system.locations-management.update', { type: props.type, location: props.data.id }))
}
...
<template>
    ...
    <q-card flat bordered class="q-mt-md">
        <q-card-section>
            <div class="text-h6 q-pb-lg">Edit {{ typeLabel }}: {{ data.name }}</div>
