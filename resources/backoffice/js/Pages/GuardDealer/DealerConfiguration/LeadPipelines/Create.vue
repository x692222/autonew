...
<template>
  ...
  <q-card flat bordered><q-card-section><div class="text-h6 q-pb-lg">Create Lead Pipeline</div><q-form @submit.prevent="submit"><q-input v-model="form.name" filled dense label="Name" maxlength="255" counter :error="!!form.errors.name" :error-message="form.errors.name" /><q-toggle v-model="form.is_default" label="Default Pipeline" class="q-mt-md" /><div class="row justify-end q-mt-lg"><div class="q-gutter-sm"><q-btn color="grey-4" text-color="standard" label="Cancel" no-wrap unelevated @click="goBack" /><q-btn color="primary" label="Save" no-wrap unelevated :loading="form.processing" :disable="form.processing" @click="submit" /></div></div></q-form></q-card-section></q-card>
</template>
