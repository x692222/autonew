...
class CustomersController extends Controller
{
    ...
    public function show(Request $request, Dealer $dealer, Customer $customer): Response
    {
        ...
        $actor = $request->user('backoffice');
        $canViewQuotations = $actor?->hasPermissionTo('editDealershipQuotations', 'backoffice') ?? false;
        $canViewInvoices = $actor?->hasPermissionTo('editDealershipInvoices', 'backoffice') ?? false;
        $canViewPayments = $actor?->hasPermissionTo('viewDealershipPayments', 'backoffice') ?? false;
        ...
        $quotationRows = $canViewQuotations
            ? $customer->quotations()
                ...
                ->map(fn ($quotation) => [
                    ...
                    'url' => route('backoffice.dealer-management.dealers.quotations.edit', ['dealer' => $dealer->id, 'quotation' => $quotation->id, 'return_to' => $request->fullUrl()]),
                ])
                ->values()
                ->all()
            : [];
        ...
        $paymentRows = $canViewPayments
            ? Payment::query()
                ...
                ->map(fn ($payment) => [
                    ...
                    'url' => route('backoffice.dealer-management.dealers.payments.show', ['dealer' => $dealer->id, 'payment' => $payment->id, 'return_to' => $request->fullUrl()]),
                ])
                ->values()
                ->all()
            : [];
